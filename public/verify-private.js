...
// ======================================================
// 🟪 UDoChain Verify Private v5.0 — STABLE
// ID handling fully delegated to backend
// ======================================================
...
async function loadPrivateValidation() {
  ...
  try {
    // 🔥 NO limpiamos ar://
    const res = await fetch(
      `${VALIDATE_API}/api/validate/storage/${encodeURIComponent(storage)}`
    );
    ...
    if (data.binaryStorageId) {
      const binaryUrl =
        `${VALIDATE_API}/api/validate/aereware/download/files/` +
        encodeURIComponent(data.binaryStorageId);
    ...
    if (data.storageId) {
      const metaUrl =
        `${VALIDATE_API}/api/validate/aereware/download/metadata/` +
        encodeURIComponent(data.storageId);
