// ======================================================
// 🔍 verify-public.js — Public Verification (Clean UI v4.5)
// ======================================================
...
document.getElementById("checkFileBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    if (hashHex === tx.replace(/^0x/, "")) {
      checkResult.textContent = "✅ Hash matches blockchain record.";
      checkResult.className = "check-result success";
    } else {
      checkResult.textContent = "❌ File does not match record.";
